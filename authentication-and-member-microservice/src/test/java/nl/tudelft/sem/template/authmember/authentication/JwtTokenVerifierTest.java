...
public class JwtTokenVerifierTest {
    ...
    public void validateNonExpiredToken() {
        String token = this.generateToken("testSecret123", "user123", -10000000L, 10000000L);
    ...
    public void validateExpiredToken() {
        String token = this.generateToken("testSecret123", "user123", -10000000L, -5000000L);
    ...
    public void validateTokenIncorrectSignature() {
        String token = this.generateToken("incorrectSecret", "user123", -10000000L, 10000000L);
        ThrowingCallable action = () -> {
            this.jwtTokenVerifier.validateToken(token);
        };
    ...
    public void parseNetid() {
        ...
        String token = this.generateToken("testSecret123", expected, -10000000L, 10000000L);
    ...
    private String generateToken(String jwtSecret, String netid, long issuanceOffset, long expirationOffset) {
        ...
        return Jwts.builder().setClaims(claims).setSubject(netid).setIssuedAt(new Date(System.currentTimeMillis() + issuanceOffset)).setExpiration(new Date(System.currentTimeMillis() + expirationOffset)).signWith(SignatureAlgorithm.HS512, jwtSecret).compact();
    }
